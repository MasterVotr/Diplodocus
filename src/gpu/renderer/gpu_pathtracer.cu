...
D float3 TracePath(GpuTraceContext trace_ctx, GpuRayContext ray_ctx) {
    ...
    while (ray_ctx.depth < max_depth) {
        ...
        // float3 pl_contrib = LocalIlluminationPointLights(trace_ctx, ray_hit);
        // bounce_state.radiance = bounce_state.radiance + bounce_state.throughput * pl_contrib;
        float3 al_contrib = LocalIlluminationAreaLights(trace_ctx, ray_ctx.pixel_x, ray_ctx.pixel_y, ray_hit);
        ...
        if (has_refl && has_refr) {
            float fresnel = SchlickFresnel(trace_ctx, ray_ctx.ray, ray_hit);
